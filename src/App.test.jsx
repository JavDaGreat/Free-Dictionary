...
describe("getDefinition function", async () => {
  it("search",async()=>{
    ...
    expect(await screen.findByText("food"
    )).toBeInTheDocument()
    expect(await screen.getByTestId("audio-btn") )
  ...
  it("If words dose not exist", async ()=>{
    ...
    const btn = screen.getByRole("button")
    await userEvent.click(btn)
    expect(screen.getByText("Sorry pal, we couldn't see any word been typed inside input section to search."
    )).toBeInTheDocument()
    screen.debug()
 });
  ...
  it("If words dose not exist", async ()=>{
    ...
    await userEvent.click(btn)
    expect(await screen.findByText("Sorry pal, we couldn't find definitions for the word you were looking for."
    )).toBeInTheDocument()
    screen.debug()
 })});
